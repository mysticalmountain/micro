 ...
 */
@Component
public class PatchExecuteComplex implements ExecuteTemplate<Request, ComplexService, Response> {
    ...
    @Override
    public Response execute(Request request, ComplexService processor, Object... args) throws ServiceException {
        long begin = System.currentTimeMillis();
        log.info("begin request method [PATCH], uri [" + String.valueOf(args[1]) + "]");
        try {
            ComplexService complexService = (ComplexService) AopTargetUtils.getTarget(processor);
            Service service = complexService.getClass().getAnnotation(Service.class);
            ...
            Response response = (Response) processor.process(request, args);
            ...
            log.info("end request method [PATCH], uri [" + args[1] + "], time mills:" + (end - begin) + " ms");
        }
    }
}
